...
$(document).ready(async function () {
    ...
    $('.nsc-open-popup-register-nimbus').on('click', function () {
        ...
        const analytic = '&utm_source=capture&utm_medium=addon&utm_campaign=sign_up'
    ...
    $('#nsc_copy_url').click(function () {
        if (nimbus_core.is_app) {
            ...
            nimbus_core.copyTextToClipboard($('#nsc_linked input').val())
        }
    })
        ...
        if (dropboxShare.getAccessToken()) {
            ...
            dropboxShare.save(blob, nimbus_screen.get.fileName(true), function (err, res) {
                if (err) {
                    ...
                    nimbus_core.copyTextToClipboard(res.path_lower)
                }
            })
        } else {
            dropboxShare.login()
                ...
                if (!is_auth) {
                    ...
                    nimbusShare.server.send[is_video ? 'screencast' : 'screenshot'](data, async function (err, res) {
                        ...
                        if (!err && res) {
                            ...
                            // await nscCore.tabCreate({ url: `${domain}/video-edit?workspaceId=${workspace.globalId}&attachmentGlobalId=${attachmentGlobalId}&blotid=${blotid}&url=${url}` })
                            ...
                            nimbus_core.copyTextToClipboard(linked)
...
chrome.runtime.onMessage.addListener(function (req) {
    ...
    if (req.operation === 'access_youtube') {
        if (youtubeShare.type === 'panel') {
            ...
            youtubeShare.save(nscFFmpeg.blob, nimbus_screen.get.fileName(true), function (err, res) {
